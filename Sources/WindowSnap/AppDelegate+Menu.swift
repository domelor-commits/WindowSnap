...
extension AppDelegate {
    ...
    func buildMenu(into menu: NSMenu) {
        ...
        // World clock glance at the bottom: current time in the zones picked in the
        // Convert tab's World Time grid (column 0 is "home" — that's the menu-bar
        // clock already, so it's skipped). Rebuilt on every open via
        // menuNeedsUpdate, so the times are always current.
